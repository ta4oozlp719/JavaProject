...
public class FriendDAO {
   public String insert(String uid, String frid) throws NamingException, SQLException, ParseException{
        ...
        try {
            ...
            if (rs.next()) return "EX"; // Existing friendship
            stmt.close();
            ...
            return (count == 1) ? "OK" : "ER"; // Success or error
        } finally {
            if (rs != null) rs.close();
   ...
   public String remove(String uid, String frid) throws NamingException, SQLException, ParseException {
       ...
       try {
           ...
           int count = stmt.executeUpdate();
           return (count == 1) ? "OK" : "ER"; // Return OK if one record is deleted, otherwise ER
       } finally {
           if (stmt != null) stmt.close();
           if (conn != null) conn.close();
       }
   ...
   public String getList(String uid) throws NamingException, SQLException, ParseException {
       ...
       try {
           ...
           if (cnt == 0) return "[]"; // Return an empty JSON array if no friends are found
           rs.close(); stmt.close();
